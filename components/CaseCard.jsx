...
export default function CaseCard({ item }) {
  function handleImageError(event) {
    ...
    event.currentTarget.src = withBasePath('/assets/case-corporate.svg');
  }

  return (
    <article className="overflow-hidden rounded-sm border border-black/10 bg-white shadow-sm">
      <img src={withBasePath(item.image)} alt={item.title} onError={handleImageError} className="h-56 w-full object-cover" />
      <div className="p-6">
